...
macro_rules! runtime_format {
    ($format:expr) => {{
        ...
        let format = $format;
        let fa = $crate::runtime_fmt::FormatArguments {
            format: AsRef::as_ref(&format),
            args: &[],
        };
        $crate::runtime_fmt::runtime_format_arguments_to_string(fa)
    }};
    ($format:expr,  $($tail:tt)* ) => {{
        let format = $format;
        let fa = $crate::runtime_fmt::FormatArguments {
...
#[doc(hidden)]
pub fn runtime_format_arguments_to_string(args: FormatArguments) -> String {
    ...
    while let Some(c) = iter.next() {
        match state {
            ...
            ParserState::Argument if c == '}' => {
                ...
                let arg_value = if let Some(argument) = argument {
                    ...
                    let arg_value = args.args.get(argument_index);
                    argument_index += 1;
                    arg_value
                };
                if let Some(arg_value) = arg_value {
                    output.push_str(&arg_value.1.to_string());
                } else {
                    log::error!("runtime_format error - missing argument in \"{}\"", args.format);
                }
...
mod tests {
    #[test]
    fn test_format() {
        assert_eq!(runtime_format!("Simple Text"), "Simple Text");
        assert_eq!(runtime_format!("Quoted {{Braces}}"), "Quoted {Braces}");
        assert_eq!(runtime_format!("ARG {}", "A0"), "ARG A0");
        assert_eq!(runtime_format!("ARG {0}", "A0"), "ARG A0");
        assert_eq!(runtime_format!("ARG {0}", 50), "ARG 50");
        assert_eq!(runtime_format!("ARG {1} {0}", "A0", "A1"), "ARG A1 A0");
        assert_eq!(runtime_format!("ARG {n} {0}", "A0", n = 5), "ARG 5 A0");
    }
}
